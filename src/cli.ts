...
async function main(): Promise<void> {
  ...
  program
    ...
    .option("--product-report", "Generate product-facing lift dashboard and summary.json")
    .option("--compare-workspace <path>", "Previous workspace to compare in the product report");
  ...
  const productReport = opts.productReport ?? false;
  ...
  try {
    const result = await evaluateSkills({
      ...
      reportOutput: output,
      productReport,
      compareWorkspace: opts.compareWorkspace,
