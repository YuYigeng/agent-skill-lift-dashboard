...
export interface EvaluateSkillsArgs {
  ...
  reportOutput?: string;
  /** Generate product-facing lift scorecards and report/summary.json. */
  productReport?: boolean;
  /** Optional previous workspace for run-over-run product comparison. */
...
export interface EvaluateSkillsResult {
  ...
  reportPath?: string;
  /** Set when `productReport` is enabled. */
  summaryPath?: string;
}
...
export async function evaluateSkills(args: EvaluateSkillsArgs): Promise<EvaluateSkillsResult> {
  ...
  if (args.report !== false) {
    const result = generateReport({
      ...
      productReport: args.productReport,
      compareWorkspace: args.compareWorkspace,
    });
