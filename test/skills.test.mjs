...
test("loadSkill reads spec files and safe attachment states", () => {
  ...
  assert.deepEqual(skill.evals[0].tags, ["revenue", "csv"]);
  assert.equal(skill.evals[0].risk, "high");
});
...
test("runEval supports complete-only provider fallback and writes artifacts", async () => {
  ...
  const evalMeta = JSON.parse(readFileSync(path.join(workspace, "iteration-1", result.slug, "eval.json"), "utf8"));
  assert.deepEqual(evalMeta.tags, ["revenue", "csv"]);
  assert.equal(evalMeta.risk, "high");
});
