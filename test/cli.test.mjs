...
test("CLI runs from YAML config and writes JSONL logs plus report artifacts", async () => {
  ...
  try {
    ...
    const { stdout } = await execFileAsync(
      process.execPath,
      ["dist/cli.js", "--config", configPath, "--product-report"],
      {
        cwd: path.resolve("."),
    ...
    assert.ok(existsSync(path.join(workspace, "iteration-1", "report", "summary.json")));
    const summary = JSON.parse(readFileSync(path.join(workspace, "iteration-1", "report", "summary.json"), "utf8"));
    assert.equal(summary.totals.evals, 1);
    assert.equal(summary.tagGroups.revenue.count, 1);
